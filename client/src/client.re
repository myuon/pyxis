let endpoint = [%bs.raw {| process.env.NODE_ENV === 'production' ? "https://ycsokjfjb3.execute-api.ap-northeast-1.amazonaws.com/dev" : "http://localhost:3000" |}];
...
let get = (api) => {
  ...
  Fetch.fetchWithInit(
    ...
    Fetch.RequestInit.make(
      ~method_=Get,
      ~headers=if (token |> Js.Option.isNone || token == Some("undefined")) {
